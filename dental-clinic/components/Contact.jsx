...
const SERVICES_LIST = ["Dental Implants","Root Canal Treatment","Braces / Orthodontics","Teeth Whitening","Tooth Extraction","Dental Cleaning","Dental Check-up","Other"];
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <p className="reveal text-darkText/55 text-[17px] max-w-xl mx-auto leading-relaxed">
            Fill in the form and we'll confirm your slot within 2 hours. Or simply call us.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 space-y-4">
            {[
              { icon:<FiPhone className="text-primary text-xl"/>, bg:"bg-blue-50", title:"Call Us", content:<a href="tel:8806333633" className="text-primary font-semibold text-[15px] hover:underline">88063 33633</a>, sub:"Mon–Sun · 9 AM – 9 PM" },
              { icon:<FiMapPin className="text-green-500 text-xl"/>, bg:"bg-green-50", title:"Address", content:<p className="text-darkText/65 text-[14px] leading-relaxed">1st Floor, Vyapari Dharmshala Complex,<br/>Beside Kamdar Petrol Pump,<br/>Gandhi Chowk, Latur — 413 512</p>, sub:null },
              { icon:<FiClock className="text-accent text-xl"/>, bg:"bg-amber-50", title:"Hours", content:<div className="space-y-1">{[["Mon – Sat","9:00 AM – 9:00 PM"],["Sunday","10:00 AM – 6:00 PM"]].map(([d,t])=><div key={d} className="flex justify-between text-[13px]"><span className="text-darkText/50">{d}</span><span className="text-darkText font-medium">{t}</span></div>)}</div>, sub:null },
            ].map((item, i) => (
              <div key={i} className="reveal bg-white rounded-2xl p-5 shadow-card border border-gray-50 flex items-start gap-4 hover:shadow-cardHover transition-shadow">
                <div className={`w-11 h-11 ${item.bg} rounded-xl flex items-center justify-center flex-shrink-0`}>{item.icon}</div>
                <div className="flex-1">
                  <p className="font-semibold text-darkText text-[14px] mb-1">{item.title}</p>
                  {item.content}
                  {item.sub && <p className="text-darkText/40 text-[12px] mt-0.5">{item.sub}</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="lg:col-span-3 reveal">
            <div className="bg-white rounded-3xl shadow-cardHover border border-gray-50 p-8 sm:p-10">
              {!submitted ? (
                <>
                  <h3 className="font-display text-2xl font-bold text-darkText mb-1">Request an Appointment</h3>
                  <p className="text-darkText/50 text-[14px] mb-7">We'll call you back to confirm within 2 hours.</p>
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[11px] font-bold text-darkText/40 mb-1.5 uppercase tracking-wider">Full Name *</label>
                      ...
                      <div>
                        <label className="block text-[11px] font-bold text-darkText/40 mb-1.5 uppercase tracking-wider">Phone *</label>
                        <input className="form-input" type="tel" placeholder="Mobile number" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} required/>
                      </div>
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-darkText/40 mb-1.5 uppercase tracking-wider">Treatment Needed *</label>
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-darkText/40 mb-1.5 uppercase tracking-wider">Message (Optional)</label>
                      <textarea className="form-input resize-none" rows={4} placeholder="Describe your concern or preferred appointment time..." value={form.message} onChange={e=>setForm({...form,message:e.target.value})} />
                    </div>
                    <button type="submit" disabled={loading}
                      className={`w-full flex items-center justify-center gap-2 font-semibold text-white py-4 rounded-2xl text-[15px] transition-all duration-300 ${loading ? "bg-primary/70 cursor-not-allowed" : "btn-shimmer shadow-glow hover:shadow-[0_8px_32px_rgba(47,128,237,0.4)] hover:scale-[1.02]"}`}>
                      {loading ? (<><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Booking...</>) : (<><FiSend/>Book Appointment</>)}
                    </button>
                  </form>
                </>
              ) : (
                <div className="flex flex-col items-center justify-center py-14 text-center">
                  ...
                  <h3 className="font-display text-2xl font-bold text-darkText mb-2">Appointment Requested!</h3>
                  <p className="text-darkText/60 text-[15px] max-w-xs leading-relaxed">
                    Thank you, <strong>{form.name}</strong>! We'll call <strong>{form.phone}</strong> within 2 hours to confirm your slot.
                  </p>
                  <button onClick={() => {setSubmitted(false);setForm({name:"",phone:"",service:"",message:""}); }}
                    className="mt-8 text-primary font-semibold text-[14px] hover:underline">
